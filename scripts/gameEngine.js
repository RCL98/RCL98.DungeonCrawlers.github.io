...
function printLives() {
	let livesTxt = document.getElementById("lives");
	var textLives = lives.toString();
	livesTxt.innerHTML = "Lives: " + textLives;
}

function printScore(score) {
	let scoreTxt = document.getElementById("score");
	var textScore = score.toString();
	scoreTxt.innerHTML = "Score: " + textScore;
}

var Player = class Player {
  constructor(pos, orientation, score, speed, hp, alive, strucked, sword, meleeRange, attackAngle, attacking) {
    this.pos = pos;
  ...
  static create(pos) {
    ...
    return new Player(pos.plus(new Vec(0, -0.5)), 1, 0,
                      new Vec(0, 0), initialPlayerHp, 1, 0, 0, 0, 0, attack);
  }
}
...
var Enemy = class Enemy {
  constructor(pos, orientation, speed, lifePoints, originalPos, walkingRange, enemyType) {
    this.pos = pos;
	this.orientation = orientation;
    this.speed = speed;
    this.hp =  lifePoints;
  ...
  static create(pos, ch) {
	if (ch == "g") {
		return new Enemy(pos, 1, new Vec(2, 0), goblinHp, pos, 3, "goblin");
	} else if (ch == "b") {
		return new Enemy(pos, 1, new Vec(2, 0), goblinHp, pos, 3, "blob");
	} else {
		return new Enemy(pos, 1, new Vec(2, 0), goblinHp, pos, 3, "eye");
	}
  }
}

Enemy.prototype.size = new Vec(0.5, 0.8);
...
var Chest = class Chest {
	constructor(pos, opened, looted) {
		this.pos = pos;
		this.opened = opened;
		this.looted = looted;
	}

	get type() { return "chest"; }

	static create(pos) {
		return new Chest(pos, 0, 0);
	}
}

Chest.prototype.size = new Vec(1, 1);
...
var levelChars = {
  ".": "floor", "?": "brokenTile", "#": "wall", "D": Door, "@": "prisoner",
  "P": Player, "g": Enemy, "b": Enemy, "e": Enemy, 
  "s": Sword, "o": Coin, "K": Key, "S": Spikes, 
  "k": Key, "^": Spikes, "C": Chest
};
...
function playerAttack(state) {
	...
	let currentEnemies = state.actors.filter(actor => actor.type = "enemy");
...
Chest.prototype.collide = function(state) {
	if (!this.opened) {
		return new State(state.level, state.actors, "playing");
	} else {
		if (!this.looted) {
...
Enemy.prototype.update = function(time, state) {
   ...
   if (state.level.touches(newPos, this.size, "wall") || 
	  ...
	  return new Enemy(newPos, this.orientation, this.speed.times(-1), this.hp, this.originalPos, this.walkingRange, this.enemyType);
   } else {
		return new Enemy(newPos, this.orientation, this.speed, this.hp, this.originalPos, this.walkingRange, this.enemyType);
   }
}
...
Chest.prototype.update = function(state) {
	if (chestKeyPickedUp) {
		this.opened = 1;
	}
	return new Chest(this.pos, this.opened, this.looted);
};
...
Player.prototype.update = function(time, state, keys) {
	if (this.alive) {
		...
		if (!this.attacking) { 
			...
			return new Player(pos, this.orientation, this.score, new Vec(xSpeed, ySpeed), playerHp, this.alive, this.strucked, this.sword, this.meleeRange, this.attackAngle, attack);
		} else { 
			if (Date.now() - playerStartAttack > 500) {
				attack = 0;
			}
			return new Player(this.pos, this.orientation, this.score, new Vec(0, 0), playerHp, this.alive, this.strucked, 
								this.sword, this.meleeRange, this.attackAngle, attack);
		}
	} 
	return new Player(this.pos, this.orientation, this.score, new Vec(0, 0), playerHp, this.alive, this.strucked, 
				this.sword, this.meleeRange, this.attackAngle, attack); 
